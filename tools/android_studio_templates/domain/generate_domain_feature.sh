...
ENTITIES_ROOT="$ROOT_DIR/lib/domain/entities"
REPOSITORIES_ROOT="$ROOT_DIR/lib/domain/repositories"
USECASE_ROOT="$ROOT_DIR/lib/domain/usecases"

TARGET_FILES=(
  "$ENTITIES_ROOT/${ENTITY_NAME}.dart"
  "$REPOSITORIES_ROOT/${FEATURE_NAME}_repository.dart"
  "$USECASE_ROOT/get_${FEATURE_NAME}_usecase.dart"
)
...
render_template \
  "$TEMPLATE_DIR/domain_entity.dart.tpl" \
  "$ENTITIES_ROOT/${ENTITY_NAME}.dart"
render_template \
  "$TEMPLATE_DIR/domain_repository.dart.tpl" \
  "$REPOSITORIES_ROOT/${FEATURE_NAME}_repository.dart"
render_template \
  "$TEMPLATE_DIR/domain_usecase.dart.tpl" \
  "$USECASE_ROOT/get_${FEATURE_NAME}_usecase.dart"
echo "Created domain skeleton for feature $FEATURE_NAME using entity $ENTITY_NAME"
