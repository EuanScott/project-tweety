...
- Put page-local helper widgets in `<feature>/widgets/<feature>_<widget>.widget.dart` as `part` files of the page library, not as extra top-level classes in the page file. They are UI only: they may read state and dispatch events, but never resolve DI, reach lower layers, or hold policy. `part` files carry no imports, so add them to the page.
